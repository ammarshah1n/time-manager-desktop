...
- **Parent deletion trigger** correctly uses `ON DELETE RESTRICT` + a `BEFORE DELETE` trigger owning promotion semantics. The FK cascade conflict described in Round 2 cannot occur under this design.[^1]
- **RLS** is correctly delegated to `private.user_workspace_ids()` as a `SECURITY DEFINER stable` function, avoiding per-row join evaluation and RLS-on-RLS cycles.[^2][^3]
- **`canonical_bucket` domain** applied to both `tasks.bucket_type` and `task_sections.canonical_bucket_type` closes the silent mismatch gap.[^4]
- **`estimate_priors`** table is defined with correct composite unique key and domain-typed `bucket_type` before the learning job ships.[^1]
