...
serve(async (_req: Request) => {
  ...
  if (fetchError) {
    console.error("[renew-graph-subscriptions] fetch error:", fetchError.message);
    return new Response(
  ...
  for (const account of accounts) {
    const { id: accountId, workspace_id, graph_subscription_id, oauth_access_token } = account;
    ...
    try {
      ...
      if (response.ok) {
        ...
        renewed++;
        console.log(`[renew-graph-subscriptions] renewed ${graph_subscription_id} for workspace ${workspace_id}`);
      ...
      console.error(`[renew-graph-subscriptions] network error for ${graph_subscription_id}:`, msg);
      failed++;
    }
  }
