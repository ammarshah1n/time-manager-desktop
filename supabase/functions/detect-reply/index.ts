...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { workspaceId, waitingItemId, fromAddress, subjectKeywords } =
      await req.json();
    ...
    if (!workspaceId || !waitingItemId || !fromAddress) {
      return new Response(
        JSON.stringify({
          error:
            "Missing required fields: workspaceId, waitingItemId, fromAddress",
        }),
    ...
    assertOwnedTenant(executiveId, workspaceId);
    ...
    const { data: candidateEmails, error: emailError } = await query;
    ...
    const keywords = Array.isArray(subjectKeywords)
      ? subjectKeywords.filter((kw): kw is string =>
        typeof kw === "string" && kw.trim().length > 0
      )
      : [];
    const matchingEmails = keywords.length === 0
      ? candidateEmails
      : (candidateEmails ?? []).filter((email) => {
    ...
    if (matchingEmails && matchingEmails.length > 0) {
      ...
      const daysWaited = Math.round(
        (repliedAt - createdAt) / (1000 * 60 * 60 * 24),
      );
