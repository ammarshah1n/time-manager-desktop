# BUILD_STATE.md — Last updated: 2026-05-04 (PA co-edit invite shipped)


> **PA co-edit invite shipped 2026-05-04** — Settings → Sharing → "Invite your PA" opens an Apple share sheet with `https://facilitated.com.au/timed/invite/<code>`. PA opens it on Mac (DMG hosted on facilitated.com.au), signs up via existing Auth, app auto-presents `AcceptInviteSheet`, and joins as `pa`. Atomic accept runs through `accept_workspace_invite()` (FOR UPDATE lock; self-accept rejected before consume; same-user resume returns `already_member=true`; errcodes P0001/P0002/P0003/P0404/22023 mapped to UX copy). RESTRICTIVE RLS policies on 15 cognitive tables deny PA SELECT while tasks/sections remain co-editable. Sidebar workspace switching clears local task/section caches before flipping active workspace. Owner can revoke pending invites; PA can leave via RLS-gated self-delete.

> **Verified 2026-05-04 PA invite end-to-end** — `swift build` ✅; `swift test` ✅ (125 tests); `swift test --filter PARoleRLSGuardTests` ✅; `deno check supabase/functions/accept-invite/index.ts` ✅; `npx supabase migration up --linked` ✅ for PA migrations 20260504100000–100006; Edge Functions `accept-invite` and `bootstrap-executive` deployed ✅; remote two-profile smoke ✅ (`runId=1777873949218-26e0e1e6`): Karen accepted as `pa`, second accept returned `already_member=true`, Karen SELECT on Yasser workspace tasks returned `2`, email_messages returned `0`, behaviour_events returned `0`, invite revoke persisted, PA self-leave deleted exactly one membership; `curl` invalid token returned `401`; different-PA used-invite returned `P0002`; own-invite returned `22023`; DMG packaged/published to facilitated repo SHA-256 `c9ceb4efa4702987cc1f088f21ff0648b0ba80903c328fb14efb271228608a89`; `graphify update .` ✅.
