...
> Current target format: 7-section CIA President's Daily Brief structure generated by Opus 4.5/4.6 with 32K extended thinking (see `~/Timed-Brain/CLAUDE.md` Phase 4.01). This doc's 4-phase structure is the earlier design. Intent (intelligence not tasks) remains correct.
...
**Status:** Edge Function `generate-morning-briefing` deployed 2026-04-14. UI delivery still via legacy `MorningInterviewPane.swift` — upgrade path is UI bridge to Supabase.
