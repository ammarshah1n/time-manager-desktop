...
struct DishMeUpSheet: View {
    ...
    private func generate() {
        let filtered = tasks.filter { !$0.isDone && context.allowedBuckets.contains($0.bucket) }
        ...
        // Map back to TimedTask using id lookup
        let taskById = Dictionary(uniqueKeysWithValues: tasks.map { ($0.id, $0) })
        stack = result.items.compactMap { taskById[$0.task.id] }
        generated = true
    }
    ...
    private func toPlanTask(_ task: TimedTask) -> PlanTask {
        PlanTask(
            ...
            priority: task.isDoFirst ? 5 : (task.dueToday ? 3 : 1),
            ...
            deferredCount: task.daysInQueue > 7 ? 3 : (task.daysInQueue > 3 ? 1 : 0),
    ...
    private func bucketTypeStr(_ b: TaskBucket) -> String {
        ...
        case .reply, .calls:             return "reply"
        case .readToday, .readThisWeek:  return "read"
        case .transit:                   return "action"
