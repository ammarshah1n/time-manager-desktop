...
**ID:** STORY-027
**Title:** Global hotkey (⌘⌥T) to start focus timer from anywhere
**Description:** Register a global macOS hotkey ⌘⌥T using CGEventTap or NSEvent.addGlobalMonitorForEvents. When triggered from any app, Timed comes to front and immediately starts a Pomodoro for the top-ranked incomplete task. Shows a HUD-style overlay confirming the timer started.

**Acceptance criteria:**
- Global hotkey ⌘⌥T is registered on app launch
- When triggered from any app: NSApp.activate(ignoringOtherApps: true), then FocusTimerModel.start() for the #1 ranked task
- A HUD overlay (NSPanel or SwiftUI overlay) shows for 2 seconds: '🍅 Started: [task title]' then fades out
- Hotkey works even when Timed window is minimised or behind other windows
- Hotkey is released/unregistered on app termination
- swift build passes

**Implementation notes:**
Use NSEvent.addGlobalMonitorForEvents(matching: .keyDown) to detect ⌘⌥T (keyCode 17 = T, modifiers .command | .option). Store the monitor token and remove in applicationWillTerminate. On trigger: DispatchQueue.main.async { NSApp.activate(); store.focusTimer.start(for: store.rankedTasks.first?.task, ...) }. HUD: create a small NSPanel (non-activating, HUD level) with a SwiftUI view that auto-dismisses after 2s using DispatchQueue.main.asyncAfter.
...
**Today's date:** 2026-03-27 16:24 ACDT

---

Work on story STORY-027 now. Implement it fully, verify it, commit it, then output <promise>TASK COMPLETE</promise>.
