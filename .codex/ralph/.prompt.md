...
**ID:** STORY-018
**Title:** Pomodoro statistics — daily counts, streaks, and 7-day sparkline
**Description:** Track how many Pomodoros are completed each day. Store a `[String: Int]` date→count log in PlannerStore (persisted to JSON). DailyOverviewCard shows: today's pomodoro count, current study streak (consecutive days with ≥1 pomodoro), and a 7-bar mini sparkline using SwiftUI Charts.

**Acceptance criteria:**
- PlannerStore has `pomodoroLog: [String: Int]` (key = 'yyyy-MM-dd') persisted to the save file
- Every time FocusTimerModel fires onTaskCompletion, PlannerStore increments today's log entry
- DailyOverviewCard shows '🍅 3 today' label
- DailyOverviewCard shows '🔥 5 day streak' (or 0 if none)
- A 7-bar Chart (SwiftUI Charts) shows last 7 days of pomodoro counts below the overview text
- swift build passes, Charts framework imported correctly

**Implementation notes:**
Import Charts framework (available in macOS 13+). Add `pomodoroLog: [String: Int]` to PlannerStore, include in Codable save. Add `recordPomodoro()` func that bumps today's key. Wire it into the onTaskCompletion callback in ContentView where FocusTimerModel.start() is called. Streak = count backwards from today while log[date] > 0. In DailyOverviewCard.swift, add a Chart { ForEach(last7Days) { BarMark(...) } } block under the existing content.
...
**Today's date:** 2026-03-27 15:21 ACDT

---

Work on story STORY-018 now. Implement it fully, verify it, commit it, then output <promise>TASK COMPLETE</promise>.
